...
static bool RemoveEnvVariableFromPath(HKEY a_hKey, const char* a_cpcPathAdd, size_t a_unStrLen)
{
	...
	while (pcEnvPlace) {
		if (pcEnvPlace[a_unStrLen] == '\0') {
			*pcEnvPlace = '\0';
			bModify = true;
			break;
		}
		else if(pcEnvPlace[a_unStrLen] == ';'){
			unRemnantLen = strlen(pcEnvPlace+a_unStrLen+1);
			memmove(pcEnvPlace, pcEnvPlace+a_unStrLen+1, unRemnantLen+1);
			bModify = true;
			break;
		}
...
int QtDeployInstaller(char* a_wlacRoot, int a_isSystem)
{
	HKEY hKey = (HKEY)0;
	int nReturn = -1;
	char vcData[REG_DATA_SIZE_MIN1+1];
	...
	nPathVarLen=snprintf(vcData,REG_DATA_SIZE_MIN1,"%s;%s",vcData, a_wlacRoot);
...
int QtDeployUninstaller(char* a_wlacRoot, int a_isSystem)
{
	HKEY hKey=(HKEY)0;
	size_t unStrLen;
	//char vcPathAdd[PATH_ENV_ADD_LEN_MIN1 + 1];
	...
	unStrLen = strlen(a_wlacRoot);

	if((!a_isSystem)||(!RemoveEnvVariableFromPath(hKey, a_wlacRoot,unStrLen))){
		...
		for (i = 0; i < cSubKeys; i++){
			...
			if(RemoveEnvVariableFromPath(hKeyLocal,a_wlacRoot,unStrLen)){
...
int QtDeployUpdater(char* a_installerDir, int a_isSystem)
{
	return 0;
}
